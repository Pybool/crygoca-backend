...
MerchantAccountsSchema.methods.isValidPassword = async function (password: string) {
  try {
...
MerchantAccountsSchema.statics.addPassword = async function (
  accountId: string,
  newPassword: string,
  confirmPassword: string
): Promise<{ status: boolean; message: string }> {
...
MerchantAccountsSchema.statics.changePassword = async function (
  accountId: string,
  oldPassword: string,
  newPassword: string,
  confirmPassword: string
...
MerchantAccountsSchema.index(
  { username: 1 },
  { collation: { locale: "en", strength: 2 } }
);

const MerchantAccounts = mongoose.model<any, IAccountModel>("merchantAccounts", MerchantAccountsSchema);
