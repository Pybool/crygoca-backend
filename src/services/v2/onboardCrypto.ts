...
export const insertLogos = async () => {
  ...
  for (let crypto of cryptodata) {
    const cryptocurrency = await Cryptocurrencies.findOne({
      symbol: crypto.symbol,
    });
    if (cryptocurrency) {
      if (!cryptocurrency.logo) {
        cryptocurrency.logo = crypto.img_url.replace(
          "https://raw.githubusercontent.com/ErikThiart/cryptocurrency-icons/master",
          "https://be.crygoca.co.uk"
        );
