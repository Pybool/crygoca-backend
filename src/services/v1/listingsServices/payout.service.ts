...
export class PayoutService {
  public static async getPayOuts(req: Xrequest) {
    try {
      ...
      const aggregationPipeline: any = [
        ...commonAggregationStages,
        {
          $match: {
            $and: [
              {
                vendorAccount: new mongoose.Types.ObjectId(userId), // Match by userId
              },
              {
                $or: [
      ...
      const payouts = await Payout.aggregate(aggregationPipeline);
      ...
      const totalPayouts = await Payout.aggregate(countAggregationPipeline);

      // Extract the total count
      const totalPayoutsCount =
        totalPayouts.length > 0 ? totalPayouts[0].totalCount : 0;
