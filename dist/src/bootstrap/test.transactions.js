...
async function runTransaction() {
    ...
    try {
        ...
        const user = await accounts_model_1.default.findOne({ username: 'Eko@1011' }).session(session);
        if (!user) {
        ...
        const order = await wallet_model_1.Wallet.findOneAndUpdate({ user: user._id }, { balance: 40000 }, { session, new: true } // ✅ Correct session placement
        );
        if (order) {
